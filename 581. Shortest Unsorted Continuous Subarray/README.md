...
- sort(nums) = vector<int> MAX(nums.size()) при заполнении слева направа посредством MAX[i] = max(MAX[i - 1], nums[i]) 
- sort(nums) = vector<int> MIN(nums.size()) при заполнении справа налева посредством MIN[i] = min(MIN[i + 1], nums[i])
