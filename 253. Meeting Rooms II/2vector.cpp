...
Если текущая встреча начинается (startTimes[i]) раньше, чем заканчивается самая ранняя уже идущая встреча (endTimes[endPointer]), значит:
...
Мы минимизируем количество комнат, используя подход "сначала освобождаем, потом добавляем". Если комната освобождается к началу новой встречи, мы ее переиспользуем.
