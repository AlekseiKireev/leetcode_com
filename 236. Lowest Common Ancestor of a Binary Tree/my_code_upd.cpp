...
void Travel(TreeNode* node, TreeNode* P, TreeNode* Q){
    ...
    const bool NodeIsParent_P_or_Q_or_PQ = (!Found_P) && (!Found_Q); // Нода W рассматривается как родитель ноды W, поэтому этот аргумент находится перед нижинми if
    ...
    // postorder проходит снизу-вверх, это гарантирует наименьшее (наибольшее по глубине) NodeIsParent_P_or_Q_or_PQ
    ...
    NodeIsParent_P_or_Q_or_PQ && /*услвоие, свойственное для LCA*/
