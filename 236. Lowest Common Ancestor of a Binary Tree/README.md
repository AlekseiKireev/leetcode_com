...
* Алгоритм начинается с поиска любого из двух узлов, чей наименьший общий предок ищется.

* Каждый раз, когда узел найден, он возвращается к своему родительскому узлу в двоичном дереве.

* Если узел получает ненулевой узел с левой стороны и ненулевой узел с правой стороны, он знает, что это самый низкий общий предок, и возвращает значение своего узла к своему родителю.

1. Т.е. первая node при PostOrder, в которую вернулись оба листа, для которых ищется LCA -- является LCA.
...
[Tushar Roy - Coding Made Simple : Lowest Common Ancestor Binary Tree](https://www.youtube.com/watch?v=13m9ZCB8gjw&ab_channel=TusharRoy-CodingMadeSimple)
