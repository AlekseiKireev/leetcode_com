...
Если foundBefore = true для некоторого узла V, это значит, что текущий узел не может быть ответом, так как не является общим родителем p и q --> обходить дерево надо сверху вниз, 
чтобы верно пометить кто является родителем, а кто нет.  
