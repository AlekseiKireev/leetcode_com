    ...
    string minWindow(string& S, string& T) {
        ...
        for(int LeftPtr = 0, RightPtr = 0; RightPtr < S.size(); ++RightPtr){ // сдвиг правого указателя
            ...
            for( ;T_Size == 0; ++LeftPtr){ // сдвиг левого указателя
                ...
                if(RightPtr - LeftPtr < Substring_Size){ // обновляем ответ
                    Substring_Size = RightPtr - LeftPtr;
        ...
        return Substring_LeftPtr == -1 ? "" : S.substr(Substring_LeftPtr, Substring_Size + 1); 
        // +1 необходим ввиду того, что LeftPtr / RightPtr указывают на грницы искомго Substring
    }
};
